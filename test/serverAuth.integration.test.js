...
(async () => {
  ...
  try {
    ...
    const cases = [
      ...
      {
        name: 'POST /api/catalog/validate requires auth in production',
        method: 'POST',
        path: '/api/catalog/validate',
        body: { entry: { provider: 'trakt', listType: 'trending', mediaType: 'movie' } },
        expect: 401,
      },
      {
        name: 'POST /api/catalogs/browse-trakt requires auth in production',
        method: 'POST',
        path: '/api/catalogs/browse-trakt',
        body: { username: 'test', traktClientId: 'fake' },
        expect: 401,
      },
      {
        name: 'POST /api/addon-catalogs requires auth in production',
        method: 'POST',
        path: '/api/addon-catalogs',
        body: { manifestUrl: 'https://example.com/manifest.json' },
        expect: 401,
      },
      {
        name: 'GET /api/health/history requires auth in production',
        method: 'GET',
        path: '/api/health/history?urls=http://127.0.0.1',
        body: null,
        expect: 401,
      },
      {
        name: 'POST /api/profile/save requires auth in production',
        method: 'POST',
        path: '/api/profile/save',
        body: { username: 'u', password: 'p', config: {} },
        expect: 401,
      },
      {
