...
console.log('\nstreamDefaults');
{
  ...
  assert(needsStreamProfileUpgrade({ streamProfile: 2 }), 'v2 needs upgrade');
  assert(needsStreamProfileUpgrade({ streamProfile: 3 }), 'v3 needs upgrade to v4');
  assert(!needsStreamProfileUpgrade({ streamProfile: 4 }), 'v4 does not need upgrade');

  const { cfg, changed } = upgradeStreamProfile({ servers: [{ label: 'A' }], labelPreset: 'standard' });
  assert(changed, 'upgrade reports changed');
  assert(cfg.autoSelect === false, 'autoSelect off — list all media sources');
  ...
  assert(cfg.streamProfile === 4, 'profile version set');
  ...
  assert(merged.streamProfile === 4, 'bumps stream profile');
