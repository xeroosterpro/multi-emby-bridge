...
function buildMiniHealthPanel(history, opts){
  ...
  if(!h.length) return '<div class="gcard-health-empty">Collecting health data — pings run every 30s</div>';
...
function startCountdown(){clearInterval(refreshTimer);countdown=30;refreshTimer=setInterval(()=>{countdown--;document.getElementById('refresh-bar').textContent='Auto-refresh in '+countdown+'s · Backend pings every 30s';if(countdown<=0){countdown=30;loadHistory();}},1000);}
