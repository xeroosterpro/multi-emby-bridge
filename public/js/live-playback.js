// Keep in sync with lib/livePlayback.js
(function () {
  ...
  function isPlaybackStalled(prev, curr, opts) {
    opts = opts || {};
    const minTicks = opts.minTicks != null ? opts.minTicks : 2 * TICKS_PER_SEC;
  ...
  function annotateBuffering(sessions, prevMap, opts) {
    opts = opts || {};
    const minStallPolls = opts.minStallPolls != null ? opts.minStallPolls : 2;
    const out = [];
    ...
    for (const s of sessions || []) {
      ...
      let stallCount = (prev && prev.stallCount) || 0;
    ...
    prevMap.forEach(function (_v, key) {
      if (!seen.has(key)) prevMap.delete(key);
    });
    nextPrev.forEach(function (v, k) { prevMap.set(k, v); });

    return out;
  }

  window.MEBLivePlayback = {
    TICKS_PER_SEC,
    liveSessionKey,
    isPlaybackStalled,
    annotateBuffering,
