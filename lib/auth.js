...
const { get: cacheGet, set: cacheSet, l1Key, l1UrlForCache } = require('./embyCache');
...
const AUTH_FAIL_TTL_MS = Math.max(
  60_000,
  parseInt(process.env.SERVER_COOLDOWN_MS || process.env.AUTH_FAIL_TTL_MS || String(15 * 60 * 1000), 10)
    || 15 * 60 * 1000,
);
const authFailUntil = new Map();
const BROWSER_UA = SHIELD_UA;

function tokenKey(server) { return server.url + '|' + (server.userId || ''); }

function applyServerCredentials(server, source) {
  if (!server || !source) return;
  if (source.apiKey) server.apiKey = source.apiKey;
  if (source.userId) server.userId = source.userId;
  if (server.apiKey) {
    tokenCache.set(tokenKey(server), { token: server.apiKey, ts: Date.now() });
  }
}

function markAuthFailed(server, reason = 'auth failed') {
  const k = tokenKey(server);
  const until = Date.now() + AUTH_FAIL_TTL_MS;
  authFailUntil.set(k, until);
  for (let i = 0; i < 3; i++) authBreaker.onFailure(k);
  logger.warn(
    { server: server.label || server.url, reason, cooldownMin: Math.round(AUTH_FAIL_TTL_MS / 60_000) },
    'server cooled down — skipping API calls',
  );
}

function isAuthFailed(server) {
  const k = tokenKey(server);
  const until = authFailUntil.get(k);
  if (!until) return false;
  if (Date.now() >= until) {
    authFailUntil.delete(k);
    return false;
  }
  return true;
}

function authCooldownRemainingMs(server) {
  const until = authFailUntil.get(tokenKey(server));
  if (!until) return 0;
  ...
  try {
    if (fs.existsSync(TOKEN_CACHE_FILE)) {
      ...
      for (const [key, entry] of Object.entries(data)) {
        if (!entry.ts || (Date.now() - entry.ts) >= TOKEN_TTL) continue;
        if (key.includes('|')) {
...
async function _authenticateByPassword(server, { logPrefix = 'Re-authenticating' } = {}) {
  if (!server.username || !server.password || !server.url) {
    return null;
  }
    ...
    for (const authUrl of authPaths) {
      ...
      try {
        resp = await fetch(authUrl, {
          ...
          agent: safeAgent,
          follow: SAFE_REDIRECT_LIMIT,
          headers: buildAuthOnlyHeaders(server._deviceUserKey || server.userId || null),
      ...
      if (resp.status === 401 || resp.status === 403) {
        console.error(`[${label}] Auth failed: HTTP ${resp.status}`);
        return null;
      }
      if (!resp.ok) continue;
      ...
      if (data.AccessToken && data.User?.Id) {
        ...
        tokenCache.set(tokenKey(enriched), { token: data.AccessToken, ts: Date.now() });
        saveTokenCache();
        console.log(`[${label}] ${logPrefix} successfully ✓`);
        return enriched;
      }
    }
  } catch (err) {
    console.error(`[${label}] Auth error:`, err.message);
  }
  return null;
}
...
async function _doReauthenticate(server) {
  ...
  const enriched = await _authenticateByPassword(server, { logPrefix: 'Re-authenticated' });
  if (enriched) {
    applyServerCredentials(server, enriched);
...
async function ensureServerCredentials(server) {
  if (server.url && server.apiKey && server.userId) return server;
  if (!server.url || !server.username || !server.password) return null;
  const result = await _authenticateByPassword(server, { logPrefix: 'Authenticated for stream lookup' });
  if (!result) markAuthFailed(server);
  return result;
}

async function prepareStreamServers(servers) {
  const enabled = (servers || []).filter(
    (s) => s && s.url && s.enabled !== false && canQueryServer(s)
  );
  const settled = await Promise.allSettled(
    enabled.map((s) => ensureServerCredentials(s))
  );
  return settled
...
async function apiFetch(server, buildUrl, timeoutMs = server._timeout || 10000) {
  ...
  if (isAuthFailed(server)) {
    const e = new Error(`auth failed for ${server.label || server.url} (cooling down)`);
    e.authFailed = true;
    e.status = 401;
    throw e;
  }
  ...
  const attempt = async () => {
    ...
    const t0 = Date.now();
    const ck = l1Key(key, l1UrlForCache(urlStr));
    ...
    try {
      ...
      if (err.status === 401 || err.status === 403) {
        if (await reauthenticate(server)) {
          try {
            const resp = await attempt();
            authBreaker.onSuccess(tokenKey(server));
            return resp;
          } catch (err2) {
            lastErr = err2;
            if (err2.status === 401 || err2.status === 403) {
              markAuthFailed(server, 'token renewed but API still unauthorized');
            }
          }
        } else {
          markAuthFailed(server, 're-authentication failed');
        }
        break;
      }
  ...
  if (lastErr && (lastErr.status === 401 || lastErr.status === 403) && !isAuthFailed(server)) {
    markAuthFailed(server, 'unauthorized after retry');
  }
  const circuitKey = tokenKey(server);
  const wasOpen = authBreaker.state(circuitKey) === 'open';
  authBreaker.onFailure(circuitKey);
  if (!wasOpen && authBreaker.state(circuitKey) === 'open') {
    logger.warn({ server: server.label || server.url, retryAfterMs: authBreaker.retryAfterMs(circuitKey) }, 'server circuit opened — backing off');
  }
  throw lastErr;
}
...
function canQueryServer(server) {
  if (isAuthFailed(server)) return false;
