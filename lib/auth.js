...
async function apiFetch(server, buildUrl, timeoutMs = server._timeout || 10000) {
  ...
  const attempt = async () => {
    ...
    const t0 = Date.now();
    const cachedBody = getCachedBody(key, urlStr);
    if (cachedBody != null) {
      recordCall({ host, label, path, cached: true, status: 200, ms: Date.now() - t0, ok: true });
    ...
    try {
      ...
      setCachedBody(key, urlStr, body);
      recordCall({ host, label, path, cached: false, status: resp.status, ms: Date.now() - t0, ok: true });
      return new fetch.Response(body, { status: resp.status, headers: { 'Content-Type': 'application/json' } });
    } catch (err) {
      recordCall({ host, label, path, cached: false, status: err.status || null, ms: Date.now() - t0, ok: false });
      throw err;
    }
  };
  let lastErr;
