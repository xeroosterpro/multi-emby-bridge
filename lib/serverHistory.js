...
function makeServerHistory(db) {
  return {
    async logCheck({ userId, serverUrl, label = null, up, responseMs = null, day = null }) {
      ...
      // Upsert daily rollup: increment checks/up_checks; avg_ms is the mean response
      // time over SUCCESSFUL checks only (down checks have no response time, so they
      // must not dilute the average) — hence the denominator is up_checks, not checks.
      await db.query(
        `INSERT INTO server_uptime_daily(user_id, server_url, label, day, checks, up_checks, avg_ms)
         ...
         ON CONFLICT (user_id, server_url, day) DO UPDATE SET
           ...
           avg_ms = CASE WHEN $6 IS NULL THEN server_uptime_daily.avg_ms
             ELSE ROUND(((COALESCE(server_uptime_daily.avg_ms,0) * server_uptime_daily.up_checks) + $6) / (server_uptime_daily.up_checks + 1)) END`,
