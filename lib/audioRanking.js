...
// Sonos Arc/Beam — TrueHD/Atmos + DTS 5.1 over eARC; no DTS:X/HD-MA or LPCM/FLAC.
const SONOS_SUPPORTS = ['atmos', 'truehd', 'ddplus', 'dts', 'dd', 'aac', 'other'];
const SONOS_FRIENDLY_ORDER = SONOS_SUPPORTS.slice();
...
const DEVICE_PRESETS = [
  ...
  { id: 'sonos',      label: 'Sonos (Arc/Beam)',        supports: SONOS_SUPPORTS,
...
const COMBO_PRESETS = [
  ...
  { id: 'appletv_earc',  label: 'Apple TV → eARC',      kind: 'combo', combo: ['appletv', 'soundbar'],
    settings: { surroundPriority: true, autoSelect: false, suggestedOrder: EARC_FRIENDLY_ORDER },
    note: 'Dolby surround over eARC; DTS/X files hidden' },
  { id: 'shield_sonos',  label: 'Shield → Sonos',       kind: 'combo', combo: ['shield', 'sonos'],
    settings: { surroundPriority: true, autoSelect: false, suggestedOrder: SONOS_FRIENDLY_ORDER },
    note: 'TrueHD/Atmos + DTS 5.1; DTS:X/HD-MA/LPCM/FLAC hidden' },
  { id: 'appletv_sonos', label: 'Apple TV → Sonos',     kind: 'combo', combo: ['appletv', 'sonos'],
    settings: { surroundPriority: true, autoSelect: false, suggestedOrder: SONOS_FRIENDLY_ORDER },
    note: 'TrueHD/Atmos + DTS 5.1; DTS:X/HD-MA/LPCM/FLAC hidden' },
];
...
function mergePresetSettings(selectedIds, deviceIds) {
  ...
  if (hasSource && hasSonos) {
    ...
    if (!suggestedOrder) suggestedOrder = SONOS_FRIENDLY_ORDER;
...
module.exports = {
  AUDIO_FORMATS, DEFAULT_ORDER, SURROUND_FRIENDLY_ORDER, EARC_FRIENDLY_ORDER, SONOS_FRIENDLY_ORDER,
  ...
  filterDisabledHide,
  SONOS_SUPPORTS, DEVICE_PRESETS, COMBO_PRESETS, AUDIO_PRESETS,
