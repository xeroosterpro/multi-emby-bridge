...
async function fetchEndpoint(server, resourcePath, limit, timeoutMs) {
  const variants = apiPathVariants(server.url, resourcePath);
  for (const variant of variants) {
    try {
      const resp = await apiFetch(
        { ...server, _timeout: timeoutMs },
        () => {
          const u = new URL(variant.toString());
          u.searchParams.set('Limit', String(limit));
          u.searchParams.set('Fields', ITEM_FIELDS);
          if (resourcePath.includes('Resume')) u.searchParams.set('MediaType', 'Video');
          return u;
        },
        timeoutMs,
      );
