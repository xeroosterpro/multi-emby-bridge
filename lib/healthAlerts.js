 ...
 * @param {string} url
 * @param {number} consecutive - default HEALTH_CONSECUTIVE_DOWN
 */
function isServerDown(healthHistory, url, consecutive = HEALTH_CONSECUTIVE_DOWN) {
...
function detectDownServers(healthHistory, servers, opts = {}) {
  const consecutive = opts.consecutive != null ? opts.consecutive : HEALTH_CONSECUTIVE_DOWN;
  const out = [];
