...
/** Compact size for Stremio description lines (no space before unit). */
...
function createRateLimiter(windowMs, maxRequests) {
  const hits = new Map();
  // Cleanup every windowMs. unref() so these timers never keep the process alive
  // (matters for clean exit on SIGTERM and for short-lived test processes).
...
// Cross-row deduplication cache (60s TTL per config key). Bounded LRU so a flood
// of distinct config strings can't grow it without limit.
const { makeLruCache } = require('./lruCache');
const _dedupCache = makeLruCache({ max: 300, ttlMs: 60000 });
function getDedupSeen(key) {
  let seen = _dedupCache.get(key);
  if (!seen) { seen = new Set(); _dedupCache.set(key, seen); }
  return seen;
}
