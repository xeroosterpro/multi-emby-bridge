...
const { titlesMatch } = require('./activityEnrich');
...
function inferLiveFromRecent(recent, opts = {}) {
  ...
  for (const entry of recent || []) {
    ...
    const server = entry.server || (entry.bestFile && entry.bestFile.label) || null;
    if (!server) continue;
...
function mergeLiveSources(lists, opts = {}) {
  ...
  // Also dedupe bridge rows when a sessions row exists for same title+server
  const out = [...map.values()];
  const sessionTitles = new Set(
    out.filter(s => s.source === 'sessions' || s.source === 'user-playing' || s.source === 'browser-sessions')
      .map(s => `${s.server}|${s.title}`)
  );
  return out.filter(s => {
    if (s.source !== 'bridge') return true;
    return !sessionTitles.has(`${s.server}|${s.title}`);
  });
}
