...
async function pingHealthServers() {
  ...
  await Promise.all(healthServers.map(async (server) => {
    const { up, ms } = await probeServerReachable(server, authByUrl);
...
loadHealthData();
setInterval(() => pingHealthServers().catch(e => console.error('[health/ping]', e.message)), HEALTH_INTERVAL_MS);
setTimeout(  () => pingHealthServers().catch(e => console.error('[health/ping]', e.message)), 10000);
