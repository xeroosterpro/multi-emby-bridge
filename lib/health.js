...
const { buildAuthOnlyHeaders } = require('./embyClient');
...
const HEALTH_PINGER_ENABLED = process.env.HEALTH_PINGER_ENABLED === '1'
  || process.env.HEALTH_PINGER_ENABLED === 'true';

// Boot: load persisted data; background pinger opt-in only
loadHealthData();
if (HEALTH_PINGER_ENABLED) {
  scheduleHealthPing();
  setTimeout(() => {
