...
async function buildDashboardBundle(ctx) {
  ...
  if (scopeNeeds(scope, 'stats')) {
    tasks.push((async () => {
      try {
        const [conn, lib] = await Promise.all([
          connectionsMod.fetchConnections(servers, healthHistory, mapPool),
          libraryMod.fetchLibraryBatch(servers, mapPool),
        ]);
        connections = conn;
        library = lib;
        ...
        errors.push({ part: 'stats', message: e.message });
      }
    })());
  }
