...
async function getRecentlyAdded(servers, type, timeoutMs = 8000) {
  const itemType = type === 'movie' ? 'Movie' : 'Series';

  const results = await Promise.allSettled(servers.map(async (server) => {
    const resp = await apiFetch(server, () => {
      const url = new URL(`${server.url}/Users/${server.userId}/Items/Latest`);
      url.searchParams.set('IncludeItemTypes', itemType);
      url.searchParams.set('Fields', 'ProviderIds,Name,Overview,ProductionYear,CommunityRating');
      url.searchParams.set('Limit', '20');
      url.searchParams.set('EnableImages', 'false');
      return url;
    }, timeoutMs);
