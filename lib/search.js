...
function raceQueries(fns) {
  return Promise.any(fns.map((fn) =>
    fn().then((r) => { if (!r.length) throw new Error('empty'); return r; })
  )).catch(() => []);
}
...
async function queryServerForEpisode(server, imdbId, season, episode) {
  ...
  const out = dedupeItems(
    perSeriesResults.flatMap((r) => (r.status === 'fulfilled' ? r.value : []))
  );
  setResolved(server, 'series', imdbId, season, episode, out);
  return out;
}
