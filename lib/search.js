...
async function queryServerForMovie(server, imdbId) {
  ...
  if (out.length) setResolved(server, 'movie', imdbId, null, null, out);
  return out;
}
...
async function queryServerForEpisode(server, imdbId, season, episode) {
  ...
  if (seriesItems.length === 0) {
    ...
    if (direct.length) setResolved(server, 'series', imdbId, season, episode, direct.slice(0, 1));
  ...
  try {
    ...
    if (eps.length) setResolved(server, 'series', imdbId, season, episode, eps);
    return eps;
  } catch {
    const direct = await queryServerForEpisodeDirect(server, imdbId, season, episode);
    if (direct.length) setResolved(server, 'series', imdbId, season, episode, direct.slice(0, 1));
