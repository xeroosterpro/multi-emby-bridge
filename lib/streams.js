...
async function getStreamsFromServer(server, type, imdbId, season, episode, labelPreset, streamOpts = {}) {
  try {
    ...
    if (item) {
      ...
      let mediaSources = embedded;
      if (!mediaSources) {
        try {
          mediaSources = await fetchPlaybackInfo(server, item.Id);
        } catch (err) {
      ...
      if (mediaSources?.length) {
        allStreams.push(...mediaSourcesToStreams(server, item.Id, mediaSources, labelPreset, streamOpts));
      }
    }
