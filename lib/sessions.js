...
const { fetchWithTimeout, authHeaders, appendAuth } = require('./auth');
...
async function fetchServerSessions(server, fetchImpl) {
  ...
  const opts = { headers: authHeaders(normalized) };
  const resp = fetchImpl
    ? await fetchImpl(url.toString(), opts)
    : await fetchWithTimeout(url.toString(), 8000, opts);
