...
const LIVE_SESSION_TIMEOUT_MS = 5000;

async function fetchJsonFromPaths(server, urlObjs, fetchImpl, timeoutMs = LIVE_SESSION_TIMEOUT_MS) {
...
async function fetchSessionsList(server, fetchImpl, withActiveFilter, timeoutMs) {
  const urls = sessionEndpointUrls(server.url).map((u) => {
    const copy = new URL(u.toString());
    if (withActiveFilter) copy.searchParams.set('ActiveWithinSeconds', '7200');
    return copy;
  });
  return fetchJsonFromPaths(server, urls, fetchImpl, timeoutMs);
}

async function fetchUserPlayingItems(server, fetchImpl, timeoutMs) {
  if (!server?.userId) return [];
  const result = await fetchJsonFromPaths(server, userPlayingEndpointUrls(server.url, server.userId), fetchImpl, timeoutMs);
...
async function fetchServerSessionsDetailed(server, fetchImpl, opts = {}) {
  ...
  const timeoutMs = opts.timeoutMs || LIVE_SESSION_TIMEOUT_MS;
  const t0 = Date.now();
  ...
  for (const withFilter of [true, false]) {
    const result = await fetchSessionsList(normalized, fetchImpl, withFilter, timeoutMs);
    if (result.ok) {
  ...
  if (!live.length && sessionsErr !== 'timeout') {
    const playing = await fetchUserPlayingItems(normalized, fetchImpl, timeoutMs);
    if (playing.length) {
      live = playing;
      method = 'user-playing';
    }
