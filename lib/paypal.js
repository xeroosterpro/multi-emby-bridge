...
// Cancel an active subscription at PayPal so billing actually stops (SEC-5).
// Resolves true on success (HTTP 204). PayPal returns 422 when the subscription
// is already cancelled/expired — surfaced via err.status so callers can treat it
// as "already done" rather than a hard failure.
async function cancelSubscription(id, reason = 'Cancelled by user') {
  const token = await getAccessToken();
  const r = await fetch(`${baseUrl()}/v1/billing/subscriptions/${id}/cancel`, {
    method: 'POST',
  ...
  const err = new Error('PayPal cancel failed: ' + r.status + (body ? ' ' + body : ''));
  err.status = r.status;
  throw err;
}
...
module.exports = { isConfigured, baseUrl, publicConfig, getAccessToken, getSubscription, cancelSubscription, verifyWebhookSignature };
