...
function getDataDictionary() {
  ...
  return { version: 2, categories: cats };
}
...
function makeAdminIntel(deps = {}) {
  ...
  async function getSparkHistories(keys) {
    const m = new Map();
    if (!db.isConfigured() || !keys.length) return m;
    try {
      const q = await db.query(`
        SELECT server_key, up, probed_at FROM (
          SELECT server_key, (payload->>'up')::boolean AS up, probed_at,
                 ROW_NUMBER() OVER (PARTITION BY server_key ORDER BY probed_at DESC) AS rn
            FROM intel_snapshots
           WHERE server_key = ANY($1) AND probed_at > now() - interval '7 days'
        ) sub WHERE rn <= 48
         ORDER BY server_key, probed_at ASC`, [keys]);
      for (const r of q.rows) {
        if (!m.has(r.server_key)) m.set(r.server_key, []);
        m.get(r.server_key).push(r.up ? 1 : 0);
      }
    } catch { /* */ }
    return m;
  }
