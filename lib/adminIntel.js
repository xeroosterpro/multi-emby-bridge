// ─── Admin Data Center: global server inventory + Emby/Jellyfin probes ──────
const fs = require('fs');
const os = require('os');
const path = require('path');
const db = require('./db');
const { apiFetch, pingServer, getEffectiveApiKey, tokenCache, BROWSER_UA } = require('./auth');
const { fetchServerSessionsDetailed } = require('./sessions');
const { snapshot: systemMetrics } = require('./metrics');
const { timeSeries, serverBreakdown, topContent } = require('./adminStats');
const { recentEvents, aggregateEvents, reauthTimeSeries } = require('./tokenEvents');
const { isDemoServer, stripDemoServers, DEMO_SERVER_SQL_EXCLUDE, DEMO_SERVER_SQL_MATCH } = require('./demoServers');

const TOKEN_TTL = 12 * 60 * 60 * 1000;
const DATA_DIR = process.env.DATA_DIR || path.join(__dirname, '..', 'data');
const TOKEN_CACHE_FILE = path.join(DATA_DIR, 'token-cache.json');
const PROBE_TIMEOUT = 6000;
const SNAPSHOT_RETENTION_DAYS = 30;
const CACHE_TTL_MS = 30000;
const SNAPSHOT_INTERVAL_MS = 5 * 60 * 1000;
const BRIDGE_METRIC_INTERVAL_MS = 60 * 1000;

function makeTTLCache(ttlMs = CACHE_TTL_MS) {
  const m = new Map();
  return {
    get(key) {
      const e = m.get(key);
      if (!e || Date.now() - e.at > ttlMs) return null;
      return e.data;
    },
    set(key, data) { m.set(key, { at: Date.now(), data }); },
    invalidate(key) {
      if (key) m.delete(key);
      else m.clear();
    },
  };
}

function summarizeProbes(probes = {}) {
  let ok = 0;
  let fail = 0;
  const details = [];
  for (const [name, p] of Object.entries(probes)) {
    const isOk = p?.ok !== false && (p?.ok === true || p?.data != null);
    if (isOk) ok++;
    else fail++;
    details.push({ name, ok: isOk, ms: p?.ms ?? null, error: p?.error || null });
  }
  const total = ok + fail;
  return {
    ok,
    fail,
    total,
    scorePct: total ? Math.round((ok / total) * 100) : null,
    details,
  };
}

function normUrl(url) {
  return String(url || '').replace(/\/+$/, '');
}

function serverIntelKey(server) {
  return normUrl(server.url) + '|' + (server.userId || '');
}

function stripSecrets(entry) {
  const o = { ...entry };
  delete o.apiKey;
  delete o.password;
  return o;
}

function readTokenCacheFile() {
  try {
    if (fs.existsSync(TOKEN_CACHE_FILE)) return JSON.parse(fs.readFileSync(TOKEN_CACHE_FILE, 'utf8'));
  } catch { /* */ }
  return {};
}

function tokenCacheEntries() {
  const file = readTokenCacheFile();
  const out = [];
  const seen = new Set();
  for (const [key, entry] of tokenCache.entries()) {
    seen.add(key);
    const [url, userId] = key.split('|');
    const ageMs = entry.ts ? Date.now() - entry.ts : null;
    out.push({
      key,
      url: url || key,
      userId: userId || '',
      cached: true,
      ageMs,
      ttlRemainingMs: ageMs != null ? Math.max(0, TOKEN_TTL - ageMs) : null,
      expiresAt: entry.ts ? new Date(entry.ts + TOKEN_TTL).toISOString() : null,
    });
  }
  for (const [key, entry] of Object.entries(file)) {
    if (seen.has(key)) continue;
    const [url, userId] = key.split('|');
    const ageMs = entry.ts ? Date.now() - entry.ts : null;
    if (ageMs != null && ageMs >= TOKEN_TTL) continue;
    out.push({
      key,
      url: url || key,
      userId: userId || '',
      cached: true,
      ageMs,
      ttlRemainingMs: ageMs != null ? Math.max(0, TOKEN_TTL - ageMs) : null,
      expiresAt: entry.ts ? new Date(entry.ts + TOKEN_TTL).toISOString() : null,
    });
  }
  return out;
}

async function safeProbe(fn) {
  const t0 = Date.now();
  try {
    const data = await fn();
    return { ok: true, ms: Date.now() - t0, data, error: null };
  } catch (e) {
    return { ok: false, ms: Date.now() - t0, data: null, error: e.status ? `HTTP ${e.status}` : (e.message || 'failed') };
  }
}

async function probeServerIntel(server) {
  const s = { ...server, url: normUrl(server.url), _timeout: PROBE_TIMEOUT };
  const probes = {};

  probes.ping = await safeProbe(async () => {
    const ms = await pingServer(s);
    if (ms == null) throw new Error('unreachable');
    return { ms };
  });

  probes.systemInfo = await safeProbe(async () => {
    const resp = await apiFetch(s, () => new URL(`${s.url}/System/Info`), PROBE_TIMEOUT);
    const d = await resp.json();
    return {
      serverName: d.ServerName || d.ProductName || null,
      version: d.Version || null,
      id: d.Id || null,
      operatingSystem: d.OperatingSystemDisplayName || d.OperatingSystem || null,
      startupWizardCompleted: d.StartupWizardCompleted,
    };
  });

  probes.scheduledTasks = await safeProbe(async () => {
    const resp = await apiFetch(s, () => new URL(`${s.url}/ScheduledTasks`), PROBE_TIMEOUT);
    const d = await resp.json();
    const tasks = (Array.isArray(d) ? d : []).slice(0, 10).map(t => ({
      name: t.Name,
      state: t.State,
      progress: t.CurrentProgressPercentage,
      lastRun: t.LastExecutionResult?.EndTimeUtc || null,
    }));
    return { count: (Array.isArray(d) ? d : []).length, running: tasks.filter(t => t.state === 'Running').length, tasks };
  });

  probes.libraryCounts = await safeProbe(async () => {
    const resp = await apiFetch(s, () => {
      const u = new URL(`${s.url}/Items/Counts`);
      u.searchParams.set('UserId', s.userId);
      return u;
    }, PROBE_TIMEOUT);
    const d = await resp.json();
    return { movies: d.MovieCount || 0, shows: d.SeriesCount || 0, episodes: d.EpisodeCount || 0 };
  });

  if (s.userId) {
    probes.userProfile = await safeProbe(async () => {
      const resp = await apiFetch(s, () => new URL(`${s.url}/Users/${encodeURIComponent(s.userId)}`), PROBE_TIMEOUT);
      const d = await resp.json();
      return {
        name: d.Name || null,
        lastActivityDate: d.LastActivityDate || null,
        isAdministrator: !!d.Policy?.IsAdministrator,
      };
    });

    probes.views = await safeProbe(async () => {
      const resp = await apiFetch(s, () => new URL(`${s.url}/Users/${encodeURIComponent(s.userId)}/Views`), PROBE_TIMEOUT);
      const d = await resp.json();
      const items = d.Items || [];
      return { count: items.length, names: items.slice(0, 12).map(v => v.Name) };
    });

    probes.latest = await safeProbe(async () => {
      const resp = await apiFetch(s, () => {
        const u = new URL(`${s.url}/Users/${encodeURIComponent(s.userId)}/Items/Latest`);
        u.searchParams.set('Limit', '5');
        u.searchParams.set('Fields', 'ProviderIds,Name,Type');
        return u;
      }, PROBE_TIMEOUT);
      const d = await resp.json();
      const items = Array.isArray(d) ? d : (d.Items || []);
      return items.map(i => ({ id: i.Id, name: i.Name, type: i.Type, imdb: i.ProviderIds?.Imdb || null }));
    });

    probes.resume = await safeProbe(async () => {
      const resp = await apiFetch(s, () => {
        const u = new URL(`${s.url}/Users/${encodeURIComponent(s.userId)}/Items/Resume`);
        u.searchParams.set('Limit', '5');
        u.searchParams.set('Fields', 'Name,Type,UserData');
        return u;
      }, PROBE_TIMEOUT);
      const d = await resp.json();
      const items = d.Items || [];
      return { count: items.length, items: items.map(i => i.Name) };
    });

    probes.favorites = await safeProbe(async () => {
      const resp = await apiFetch(s, () => {
        const u = new URL(`${s.url}/Users/${encodeURIComponent(s.userId)}/Items`);
        u.searchParams.set('Filters', 'IsFavorite');
        u.searchParams.set('Limit', '5');
        u.searchParams.set('Fields', 'Name,Type');
        return u;
      }, PROBE_TIMEOUT);
      const d = await resp.json();
      const items = d.Items || [];
      return { count: items.length, items: items.map(i => i.Name) };
    });

    probes.playedItems = await safeProbe(async () => {
      const resp = await apiFetch(s, () => {
        const u = new URL(`${s.url}/Users/${encodeURIComponent(s.userId)}/PlayedItems`);
        u.searchParams.set('Limit', '10');
        u.searchParams.set('Fields', 'Name,Type,UserData,ProviderIds');
        return u;
      }, PROBE_TIMEOUT);
      const d = await resp.json();
      const items = d.Items || [];
      return items.map(i => ({
        name: i.Name,
        type: i.Type,
        playedPct: i.UserData?.PlayedPercentage,
        lastPlayed: i.UserData?.LastPlayedDate,
      }));
    });

    probes.itemsSearch = await safeProbe(async () => {
      const resp = await apiFetch(s, () => {
        const u = new URL(`${s.url}/Users/${encodeURIComponent(s.userId)}/Items`);
        u.searchParams.set('SearchTerm', 'the');
        u.searchParams.set('Limit', '5');
        u.searchParams.set('Recursive', 'true');
        u.searchParams.set('Fields', 'Name,Type,ProductionYear');
        return u;
      }, PROBE_TIMEOUT);
      const d = await resp.json();
      const items = d.Items || [];
      return {
        totalRecordCount: d.TotalRecordCount ?? items.length,
        sample: items.map(i => ({ name: i.Name, type: i.Type, year: i.ProductionYear })),
      };
    });
  }

  const sess = await fetchServerSessionsDetailed(s, null, { timeoutMs: PROBE_TIMEOUT });
  probes.sessions = {
    ok: sess.ok,
    ms: sess.ms,
    error: sess.error,
    data: {
      count: sess.count,
      method: sess.method,
      live: (sess.live || []).slice(0, 5).map(l => ({
        title: l.title, progressPct: l.progressPct, client: l.client, isTranscoding: l.isTranscoding,
      })),
    },
  };

  const latestItem = probes.latest?.ok && probes.latest.data?.[0];
  if (latestItem?.id && s.userId) {
    probes.playbackSample = await safeProbe(async () => {
      const resp = await apiFetch(s, () => {
        const u = new URL(`${s.url}/Items/${latestItem.id}/PlaybackInfo`);
        u.searchParams.set('UserId', s.userId);
        return u;
      }, PROBE_TIMEOUT);
      const d = await resp.json();
      const src = (d.MediaSources || [])[0];
      if (!src) return { note: 'no media sources' };
      const video = (src.MediaStreams || []).find(m => m.Type === 'Video');
      return {
        sampleTitle: latestItem.name,
        size: src.Size,
        bitrate: src.Bitrate,
        container: src.Container,
        video: video ? { codec: video.Codec, width: video.Width, height: video.Height, hdr: video.VideoRangeType } : null,
      };
    });
  }

  const cacheKey = serverIntelKey(s);
  const cacheEntry = tokenCache.get(cacheKey);
  const effectiveKey = getEffectiveApiKey(s);
  const tokenMeta = {
    hasStoredKey: !!s.apiKey,
    hasCachedToken: !!cacheEntry,
    cacheAgeMs: cacheEntry?.ts ? Date.now() - cacheEntry.ts : null,
    ttlRemainingMs: cacheEntry?.ts ? Math.max(0, TOKEN_TTL - (Date.now() - cacheEntry.ts)) : null,
    usingCache: effectiveKey !== s.apiKey,
    hasReauthCredentials: !!(s.username && s.password),
  };

  const up = probes.ping.ok || probes.systemInfo.ok;
  return {
    key: cacheKey,
    probedAt: new Date().toISOString(),
    up,
    probes,
    token: tokenMeta,
  };
}

function fieldStatus(f) {
  if (f.status) return f.status;
  if (f.type === 'n/a') return 'n/a';
  if (f.type === 'restricted') return 'blocked';
  return 'collected';
}

function getDataDictionary() {
  const cats = [
    {
      id: 'identity',
      label: 'Server identity',
      fields: [
        { field: 'label', type: 'string', source: 'user_config.servers', sensitive: false, status: 'collected' },
        { field: 'url', type: 'string', source: 'user_config.servers', sensitive: false, status: 'collected' },
        { field: 'type', type: 'emby|jellyfin', source: 'user_config.servers', sensitive: false, status: 'collected' },
        { field: 'ownerUsername', type: 'string', source: 'users.username', sensitive: false, status: 'collected' },
      ],
    },
    {
      id: 'reachability',
      label: 'Reachability',
      fields: [
        { field: 'probes.ping.ms', type: 'number', source: 'GET /System/Ping', sensitive: false, status: 'collected' },
        { field: 'probes.systemInfo.version', type: 'string', source: 'GET /System/Info', sensitive: false, status: 'collected' },
        { field: 'probes.scheduledTasks', type: 'array', source: 'GET /ScheduledTasks', sensitive: false, status: 'collected' },
        { field: 'up', type: 'boolean', source: 'derived', sensitive: false, status: 'collected', notes: 'ping or systemInfo succeeded' },
      ],
    },
    {
      id: 'library',
      label: 'Library',
      fields: [
        { field: 'probes.libraryCounts.movies', type: 'number', source: 'GET /Items/Counts', sensitive: false, status: 'collected' },
        { field: 'probes.libraryCounts.shows', type: 'number', source: 'GET /Items/Counts', sensitive: false, status: 'collected' },
        { field: 'probes.libraryCounts.episodes', type: 'number', source: 'GET /Items/Counts', sensitive: false, status: 'collected' },
        { field: 'probes.views.names', type: 'string[]', source: 'GET /Users/{id}/Views', sensitive: false, status: 'collected' },
        { field: 'probes.latest', type: 'array', source: 'GET /Users/{id}/Items/Latest', sensitive: false, status: 'collected' },
        { field: 'probes.itemsSearch', type: 'object', source: 'GET /Users/{id}/Items?SearchTerm', sensitive: false, status: 'collected' },
        { field: 'probes.resume', type: 'object', source: 'GET /Users/{id}/Items/Resume', sensitive: false, status: 'collected' },
        { field: 'probes.favorites', type: 'object', source: 'GET /Users/{id}/Items?Filters=IsFavorite', sensitive: false, status: 'collected' },
      ],
    },
    {
      id: 'sessions',
      label: 'Live sessions',
      fields: [
        { field: 'probes.sessions.method', type: 'sessions|user-playing|null', source: 'GET /Sessions or Items?IsPlaying', sensitive: false, status: 'collected' },
        { field: 'probes.sessions.count', type: 'number', source: 'GET /Sessions', sensitive: false, status: 'collected', notes: 'Often blocked on reseller hosts — falls back to user-playing' },
        { field: 'probes.sessions.live', type: 'array', source: 'GET /Sessions', sensitive: false, status: 'collected' },
        { field: 'probes.playbackSample', type: 'object', source: 'GET /Items/{id}/PlaybackInfo', sensitive: false, status: 'collected' },
      ],
    },
    {
      id: 'tokens',
      label: 'Tokens & auth',
      fields: [
        { field: 'token.cacheAgeMs', type: 'number', source: 'token-cache.json', sensitive: false, status: 'collected' },
        { field: 'token.ttlRemainingMs', type: 'number', source: 'derived (12h TTL)', sensitive: false, status: 'collected' },
        { field: 'token.hasReauthCredentials', type: 'boolean', source: 'user_config.servers', sensitive: false, status: 'collected' },
        { field: 'reauthEvents', type: 'array', source: 'token_events table', sensitive: false, status: 'collected' },
      ],
    },
    {
      id: 'activity',
      label: 'Bridge activity',
      fields: [
        { field: 'requests24h', type: 'number', source: 'request_log', sensitive: false, status: 'collected' },
        { field: 'successRate', type: 'percent', source: 'request_log', sensitive: false, status: 'collected' },
        { field: 'avgResponseMs', type: 'number', source: 'request_log.response_ms', sensitive: false, status: 'collected' },
        { field: 'errorRate', type: 'percent', source: 'derived', sensitive: false, status: 'collected', notes: 'failed / total per bucket' },
      ],
    },
    {
      id: 'health',
      label: 'Health probes',
      fields: [
        { field: 'uptimePct', type: 'percent', source: 'server_uptime_daily', sensitive: false, status: 'collected' },
        { field: 'avgMs', type: 'number', source: 'server_health_log', sensitive: false, status: 'collected' },
        { field: 'sparkHistory', type: 'number[]', source: 'intel_snapshots', sensitive: false, status: 'collected', notes: '7d up/down series per server' },
      ],
    },
    {
      id: 'bridge',
      label: 'Bridge host',
      fields: [
        { field: 'cpuPercent', type: 'number', source: 'lib/metrics.js', sensitive: false, status: 'collected' },
        { field: 'sysMemPct', type: 'number', source: 'lib/metrics.js', sensitive: false, status: 'collected' },
        { field: 'rssBytes', type: 'number', source: 'process.memoryUsage', sensitive: false, status: 'collected' },
        { field: 'deployment.node', type: 'string', source: 'process.version', sensitive: false, status: 'collected' },
        { field: 'deployment.region', type: 'string', source: 'env', sensitive: false, status: 'collected' },
      ],
    },
    {
      id: 'unavailable',
      label: 'Requires admin token (documented N/A)',
      fields: [
        { field: 'allUsers', type: 'n/a', source: 'GET /Users', sensitive: true, status: 'n/a', notes: 'User token cannot list all users' },
        { field: 'activityLog', type: 'n/a', source: 'GET /ActivityLog', sensitive: true, status: 'n/a' },
        { field: 'plugins', type: 'n/a', source: 'GET /Plugins', sensitive: false, status: 'n/a' },
        { field: 'liveTv', type: 'n/a', source: 'GET /LiveTv/*', sensitive: false, status: 'n/a', notes: 'Only when Live TV configured' },
        { field: 'filePath', type: 'restricted', source: 'Items.Path', sensitive: true, status: 'blocked', notes: 'Not exposed in admin UI' },
      ],
    },
  ];
  for (const cat of cats) {
    for (const f of cat.fields) f.status = fieldStatus(f);
  }
  return { version: 3, categories: cats };
}

function makeAdminIntel(deps = {}) {
  const userConfig = deps.userConfig;
  const requestLog = deps.requestLog;
  const getRequestLog = deps.getRequestLog || (() => []);
  const liveSessions = deps.liveSessions;

  let _lastSnapshotAt = null;
  let _refreshInFlight = null;
  let _hydrated = false;
  const _cache = makeTTLCache(CACHE_TTL_MS);

  async function hydrateLastSnapshotAt() {
    if (_hydrated || !db.isConfigured()) return;
    _hydrated = true;
    try {
      const q = await db.query('SELECT MAX(probed_at) AS last_probed FROM intel_snapshots');
      if (q.rows[0]?.last_probed) _lastSnapshotAt = new Date(q.rows[0].last_probed).toISOString();
    } catch { /* */ }
  }

  async function getIntelMeta() {
    await hydrateLastSnapshotAt();
    let rowCounts = { snapshots: 0, token_events: 0, bridge_metrics: 0 };
    let lastProbed = null;
    if (db.isConfigured()) {
      try {
        const q = await db.query(`
          SELECT
            (SELECT COUNT(*)::int FROM intel_snapshots) AS snapshots,
            (SELECT COUNT(*)::int FROM token_events) AS token_events,
            (SELECT COUNT(*)::int FROM bridge_metrics) AS bridge_metrics,
            (SELECT MAX(probed_at) FROM intel_snapshots) AS last_probed`);
        rowCounts = q.rows[0] || rowCounts;
        lastProbed = q.rows[0]?.last_probed || null;
      } catch { /* */ }
    }
    return {
      scheduler: {
        snapshotIntervalMs: SNAPSHOT_INTERVAL_MS,
        bridgeIntervalMs: BRIDGE_METRIC_INTERVAL_MS,
        cacheTtlMs: CACHE_TTL_MS,
      },
      lastSnapshotAt: _lastSnapshotAt || (lastProbed ? new Date(lastProbed).toISOString() : null),
      refreshInFlight: !!_refreshInFlight,
      database: db.isConfigured(),
      rowCounts,
      at: new Date().toISOString(),
    };
  }

  let _demoPurgeDone = false;

  async function purgeDemoIntelSnapshots() {
    if (!db.isConfigured()) return 0;
    try {
      const q = await db.query(
        `DELETE FROM intel_snapshots WHERE (${DEMO_SERVER_SQL_MATCH}) RETURNING server_key`);
      return q.rowCount || 0;
    } catch (e) {
      console.error('[adminIntel/purgeDemoSnapshots]', e.message);
      return 0;
    }
  }

  async function purgeDemoServersFromAllUsers() {
    if (!db.isConfigured() || !userConfig) return { purged: 0, users: 0, snapshots: 0 };
    try {
      const q = await db.query(
        `SELECT user_id FROM user_config WHERE jsonb_array_length(config_json->'servers') > 0`);
      let purged = 0;
      let users = 0;
      for (const row of q.rows) {
        let cur;
        try { cur = await userConfig.getEditable(row.user_id); } catch { continue; }
        const servers = cur?.config?.servers;
        if (!Array.isArray(servers) || !servers.length) continue;
        const cleaned = stripDemoServers(servers);
        if (cleaned.length === servers.length) continue;
        await userConfig.save(row.user_id, { ...(cur.config || {}), servers: cleaned });
        purged += servers.length - cleaned.length;
        users++;
      }
      const snapshots = await purgeDemoIntelSnapshots();
      if (purged || snapshots) {
        console.log(`[adminIntel] removed ${purged} demo server(s) from ${users} user account(s), ${snapshots} snapshot row(s)`);
        _cache.invalidate();
      }
      _demoPurgeDone = true;
      return { purged, users, snapshots };
    } catch (e) {
      console.error('[adminIntel/purgeDemo]', e.message);
      return { purged: 0, users: 0, snapshots: 0 };
    }
  }

  async function ensureDemoServersPurged() {
    if (_demoPurgeDone) return { skipped: true };
    return purgeDemoServersFromAllUsers();
  }

  async function buildInventory() {
    if (!db.isConfigured() || !userConfig) return [];
    const q = await db.query(
      `SELECT u.id, u.username FROM users u
        JOIN user_config uc ON uc.user_id = u.id
       WHERE jsonb_array_length(uc.config_json->'servers') > 0`);
    const byKey = new Map();
    for (const row of q.rows) {
      let cfg;
      try { cfg = await userConfig.getForServe(row.id); } catch { continue; }
      for (const s of (cfg?.servers || [])) {
        if (!s?.url || !s?.apiKey || !s?.userId) continue;
        if (isDemoServer(s)) continue;
        const entry = stripSecrets({
          key: serverIntelKey(s),
          bridgeUserId: row.id,
          ownerUsername: row.username,
          label: s.label || '',
          url: normUrl(s.url),
          type: s.type || 'emby',
          embyUserId: s.userId,
          enabled: s.enabled !== false,
          hasReauthCredentials: !!(s.username && s.password),
          username: s.username || null,
          _server: s,
        });
        if (!byKey.has(entry.key)) byKey.set(entry.key, entry);
      }
    }
    return [...byKey.values()];
  }

  async function saveSnapshot(entry, payload) {
    if (!db.isConfigured()) return;
    await db.query(
      `INSERT INTO intel_snapshots(server_key, user_id, server_url, label, payload, probed_at)
       VALUES($1,$2,$3,$4,$5,now())`,
      [entry.key, entry.bridgeUserId, entry.url, entry.label, JSON.stringify(payload)]
    );
  }

  async function getLatestSnapshots() {
    if (!db.isConfigured()) return new Map();
    try {
      const q = await db.query(`
        SELECT DISTINCT ON (server_key) server_key, payload, probed_at
          FROM intel_snapshots
         ORDER BY server_key, probed_at DESC`);
      const m = new Map();
      for (const r of q.rows) m.set(r.server_key, { payload: r.payload, probedAt: r.probed_at });
      return m;
    } catch {
      return new Map();
    }
  }

  async function runSnapshotCycle() {
    const inventory = await buildInventory();
    const limit = 4;
    let i = 0;
    async function worker(batch) {
      await Promise.all(batch.map(async (entry) => {
        try {
          const payload = await probeServerIntel(entry._server);
          await saveSnapshot(entry, payload);
        } catch (e) {
          console.error('[adminIntel/probe]', entry.label || entry.url, e.message);
        }
      }));
    }
    while (i < inventory.length) {
      await worker(inventory.slice(i, i + limit));
      i += limit;
    }
    _lastSnapshotAt = new Date().toISOString();
    _cache.invalidate();
    await pruneSnapshots();
    return { probed: inventory.length, at: _lastSnapshotAt };
  }

  async function pruneSnapshots() {
    if (!db.isConfigured()) return;
    try {
      await db.query(
        `DELETE FROM intel_snapshots WHERE probed_at < now() - ($1::int || ' days')::interval`,
        [SNAPSHOT_RETENTION_DAYS]);
      await db.query(
        `DELETE FROM token_events WHERE created_at < now() - interval '90 days'`);
      await db.query(
        `DELETE FROM bridge_metrics WHERE sampled_at < now() - interval '7 days'`);
    } catch { /* */ }
  }

  async function saveBridgeMetric() {
    if (!db.isConfigured()) return;
    const m = systemMetrics();
    try {
      await db.query(
        `INSERT INTO bridge_metrics(cpu_percent, sys_mem_pct, rss_bytes, heap_used_bytes, load_avg1, uptime_sec)
         VALUES($1,$2,$3,$4,$5,$6)`,
        [m.cpuPercent, m.sysMemPct, m.rssBytes, m.heapUsedBytes, m.loadAvg1, m.uptimeSec]);
    } catch (e) { console.error('[adminIntel/bridgeMetric]', e.message); }
  }

  async function getServerUsageStats() {
    if (!db.isConfigured()) return { byKey: {}, byLabel: {} };
    try {
      const q = await db.query(`
        SELECT best_server, COUNT(*)::int AS n,
               COUNT(*) FILTER (WHERE found)::int AS found_n,
               ROUND(AVG(response_ms))::int AS avg_ms
          FROM request_log
         WHERE ts > now() - interval '7 days' AND best_server IS NOT NULL
         GROUP BY best_server`);
      const byLabel = {};
      for (const r of q.rows) {
        byLabel[r.best_server] = { requests7d: r.n, found7d: r.found_n, avgMs: r.avg_ms };
      }
      return { byLabel };
    } catch {
      return { byLabel: {} };
    }
  }

  async function getSparkHistories(keys, opts = {}) {
    const m = new Map();
    if (!db.isConfigured() || !keys.length) return m;
    const limit = opts.limit || 48;
    const hours = opts.hours || 168;
    try {
      const q = await db.query(`
        SELECT server_key, up, ping_ms, probed_at FROM (
          SELECT server_key,
                 (payload->>'up')::boolean AS up,
                 (payload->'probes'->'ping'->'data'->>'ms')::int AS ping_ms,
                 probed_at,
                 ROW_NUMBER() OVER (PARTITION BY server_key ORDER BY probed_at DESC) AS rn
            FROM intel_snapshots
           WHERE server_key = ANY($1) AND probed_at > now() - ($2::int || ' hours')::interval
        ) sub WHERE rn <= $3
         ORDER BY server_key, probed_at ASC`, [keys, hours, limit]);
      for (const r of q.rows) {
        if (!m.has(r.server_key)) m.set(r.server_key, []);
        const row = opts.includeMeta
          ? { at: r.probed_at, up: !!r.up, pingMs: r.ping_ms }
          : (r.up ? 1 : 0);
        m.get(r.server_key).push(row);
      }
    } catch { /* */ }
    return m;
  }

  async function getRecentActivity(limit = 25) {
    if (!db.isConfigured()) return [];
    try {
      const q = await db.query(
        `SELECT rl.ts, rl.content_name, rl.type, rl.season, rl.episode, rl.best_server,
                rl.found, rl.response_ms, u.username
           FROM request_log rl LEFT JOIN users u ON u.id = rl.user_id
          ORDER BY rl.ts DESC LIMIT $1`, [Math.min(100, Math.max(1, limit))]);
      return q.rows.map(r => ({
        ts: r.ts,
        title: r.content_name,
        type: r.type,
        season: r.season,
        episode: r.episode,
        server: r.best_server,
        found: !!r.found,
        ms: r.response_ms,
        username: r.username,
      }));
    } catch {
      return [];
    }
  }

  async function getFleetHeatmap(hours = 24) {
    const inventory = await buildInventory();
    const keys = inventory.map(e => e.key);
    const cellsByKey = await getSparkHistories(keys, { limit: 48, hours, includeMeta: true });
    return {
      hours,
      servers: inventory.map(e => ({
        key: e.key,
        label: e.label || e.url,
        url: e.url,
        cells: cellsByKey.get(e.key) || [],
      })),
      at: new Date().toISOString(),
    };
  }

  async function getBusinessMetrics() {
    if (!db.isConfigured()) return {};
    try {
      const u = await db.query(
        `SELECT COUNT(*)::int AS total,
                COUNT(*) FILTER (WHERE u.role='admin')::int AS admins,
                COUNT(*) FILTER (WHERE s.status IN ('active','comped'))::int AS active,
                COUNT(*) FILTER (WHERE s.status='comped')::int AS comped
           FROM users u LEFT JOIN subscriptions s ON s.user_id=u.id`);
      const rev = await db.query(
        `SELECT COALESCE(SUM(amount) FILTER (WHERE paid_at > now() - interval '30 days'),0) AS monthly,
                COALESCE(SUM(amount),0) AS lifetime FROM payments`);
      const renew = await db.query(
        `SELECT COUNT(*)::int AS n FROM subscriptions
          WHERE status='active' AND current_period_end IS NOT NULL
            AND current_period_end BETWEEN now() AND now() + interval '7 days'`);
      const failed = await db.query(`SELECT COUNT(*)::int AS n FROM payments WHERE status <> 'completed'`);
      const active24 = await db.query(
        `SELECT COUNT(DISTINCT user_id)::int AS n FROM request_log
          WHERE user_id IS NOT NULL AND ts > now() - interval '24 hours'`);
      let tickets = { open: 0, in_progress: 0, closed: 0, total: 0 };
      try {
        const tq = await db.query(`SELECT status, COUNT(*)::int AS n FROM tickets GROUP BY status`);
        for (const row of tq.rows) {
          tickets.total += row.n;
          if (row.status === 'open') tickets.open = row.n;
          else if (row.status === 'in_progress') tickets.in_progress = row.n;
          else if (['closed', 'resolved'].includes(row.status)) tickets.closed += row.n;
        }
      } catch { /* */ }
      return {
        users: u.rows[0] || {},
        revenue: { monthly: Number(rev.rows[0]?.monthly || 0), lifetime: Number(rev.rows[0]?.lifetime || 0), currency: 'USD' },
        tickets,
        upcomingRenewals: renew.rows[0]?.n || 0,
        failedPayments: failed.rows[0]?.n || 0,
        activeUsers24h: active24.rows[0]?.n || 0,
      };
    } catch {
      return {};
    }
  }

  async function getHealthRollup() {
    if (!db.isConfigured()) return { servers: [], series: [] };
    try {
      const daily = await db.query(`
        SELECT server_url, label,
               SUM(checks)::int AS checks,
               SUM(up_checks)::int AS up_checks,
               ROUND(AVG(avg_ms))::int AS avg_ms
          FROM server_uptime_daily
         WHERE day > CURRENT_DATE - 30
           AND ${DEMO_SERVER_SQL_EXCLUDE}
         GROUP BY server_url, label
         ORDER BY checks DESC`);
      const servers = daily.rows.map(r => ({
        url: r.server_url,
        label: r.label,
        uptimePct: r.checks ? Math.round((r.up_checks / r.checks) * 100) : null,
        avgMs: r.avg_ms,
        checks: r.checks,
      }));
      const series = await db.query(`
        SELECT day::text AS label,
               SUM(checks)::int AS checks,
               SUM(up_checks)::int AS up_checks
          FROM server_uptime_daily
         WHERE day > CURRENT_DATE - 30
           AND ${DEMO_SERVER_SQL_EXCLUDE}
         GROUP BY day ORDER BY day`);
      return { servers, series: series.rows };
    } catch {
      return { servers: [], series: [] };
    }
  }

  async function getOverview() {
    const cached = _cache.get('overview');
    if (cached) return cached;
    const snapshots = await getLatestSnapshots();
    const inventory = await buildInventory();
    const tokenAgg = await aggregateEvents();
    const usage = await getServerUsageStats();
    const business = await getBusinessMetrics();
    let platform = {};
    if (db.isConfigured()) {
      const u = await db.query('SELECT COUNT(*)::int AS total FROM users');
      const r24 = await db.query(`SELECT COUNT(*)::int AS n FROM request_log WHERE ts > now() - interval '24 hours'`);
      const found24 = await db.query(`SELECT COUNT(*)::int AS n FROM request_log WHERE ts > now() - interval '24 hours' AND found`);
      const avgMs = await db.query(`SELECT ROUND(AVG(response_ms))::int AS avg FROM request_log WHERE ts > now() - interval '24 hours' AND response_ms IS NOT NULL`);
      platform = {
        users: u.rows[0]?.total || 0,
        requests24h: r24.rows[0]?.n || 0,
        successRate: r24.rows[0]?.n ? Math.round((found24.rows[0]?.n / r24.rows[0]?.n) * 100) : null,
        avgResponseMs: avgMs.rows[0]?.avg ?? null,
      };
    }
    const down = [];
    const authIssues = [];
    let upCount = 0;
    let liveSessions = 0;
    for (const e of inventory) {
      const snap = snapshots.get(e.key);
      if (snap?.payload?.up === true) upCount++;
      if (snap?.payload?.up === false) down.push({ key: e.key, label: e.label, url: e.url });
      if (!e.hasReauthCredentials) authIssues.push({ key: e.key, label: e.label });
      liveSessions += snap?.payload?.probes?.sessions?.data?.count || 0;
    }
    const alerts = [];
    if (down.length) alerts.push({ level: 'error', text: `${down.length} server(s) down or unreachable`, tab: 'servers' });
    if (tokenAgg.fail24h) alerts.push({ level: 'warn', text: `${tokenAgg.fail24h} re-auth failure(s) in 24h`, tab: 'tokens' });
    if (authIssues.length) alerts.push({ level: 'info', text: `${authIssues.length} server(s) missing saved credentials for auto re-auth`, tab: 'tokens' });
    if (business.failedPayments) alerts.push({ level: 'warn', text: `${business.failedPayments} failed payment(s) on record`, tab: 'overview' });
    if (business.tickets?.open) alerts.push({ level: 'info', text: `${business.tickets.open} open support ticket(s)`, tab: 'overview' });
    const out = {
      snapshotAt: _lastSnapshotAt,
      snapshotCount: snapshots.size,
      serverCount: inventory.length,
      upCount,
      downCount: down.length,
      liveSessions,
      tokenCacheCount: tokenCacheEntries().length,
      tokenEvents: tokenAgg,
      platform,
      business,
      usage: usage.byLabel,
      alerts,
      down,
    };
    _cache.set('overview', out);
    return out;
  }

  async function getStatus() {
    await hydrateLastSnapshotAt();
    const overview = await getOverview();
    let liveCount = 0;
    if (liveSessions && db.isConfigured()) {
      try {
        const live = await getLiveIntel();
        liveCount = live.count || 0;
      } catch { /* */ }
    }
    return {
      badges: {
        down: overview.downCount || 0,
        tokenFail24h: overview.tokenEvents?.fail24h || 0,
        live: liveCount || overview.liveSessions || 0,
        alerts: (overview.alerts || []).length,
      },
      overview: {
        serverCount: overview.serverCount,
        upCount: overview.upCount,
        downCount: overview.downCount,
        successRate: overview.platform?.successRate,
        requests24h: overview.platform?.requests24h,
      },
      meta: await getIntelMeta(),
      at: new Date().toISOString(),
    };
  }

  async function getDashboard() {
    await ensureDemoServersPurged();
    const [overview, activity, servers, meta, recentActivity, heatmap] = await Promise.all([
      getOverview(),
      getActivityIntel('7d'),
      getServers(),
      getIntelMeta(),
      getRecentActivity(20),
      getFleetHeatmap(24),
    ]);
    return {
      overview,
      activity,
      servers,
      recentActivity,
      heatmap,
      meta,
      at: new Date().toISOString(),
    };
  }

  async function getServers() {
    const cached = _cache.get('servers');
    if (cached) return cached;
    const inventory = await buildInventory();
    const snapshots = await getLatestSnapshots();
    const usage = await getServerUsageStats();
    const health = await getHealthRollup();
    const healthByUrl = new Map(health.servers.map(h => [normUrl(h.url), h]));
    const keys = inventory.map(e => e.key);
    const sparks = await getSparkHistories(keys);
    const rows = inventory.map(e => {
      const snap = snapshots.get(e.key);
      const p = snap?.payload || {};
      const lib = p.probes?.libraryCounts?.data;
      const usageRow = usage.byLabel[e.label] || {};
      const healthRow = healthByUrl.get(e.url);
      const probeSummary = summarizeProbes(p.probes);
      return {
        ...e,
        probedAt: snap?.probedAt || p.probedAt || null,
        up: p.up,
        pingMs: p.probes?.ping?.data?.ms ?? null,
        version: p.probes?.systemInfo?.data?.version ?? null,
        serverName: p.probes?.systemInfo?.data?.serverName ?? null,
        movies: lib?.movies ?? null,
        shows: lib?.shows ?? null,
        episodes: lib?.episodes ?? null,
        sessions: p.probes?.sessions?.data?.count ?? 0,
        sessionMethod: p.probes?.sessions?.data?.method ?? null,
        tokenTtlMs: p.token?.ttlRemainingMs ?? null,
        requests7d: usageRow.requests7d || 0,
        avgMs: usageRow.avgMs ?? null,
        uptimePct: healthRow?.uptimePct ?? null,
        sparkHistory: sparks.get(e.key) || [],
        probeScore: probeSummary.scorePct,
        probeOk: probeSummary.ok,
        probeFail: probeSummary.fail,
      };
    });
    _cache.set('servers', rows);
    return rows;
  }

  async function getServerDetail(key) {
    const inventory = await buildInventory();
    const entry = inventory.find(e => e.key === key);
    if (!entry) return null;
    const snapshots = await getLatestSnapshots();
    const snap = snapshots.get(key);
    let payload = snap?.payload;
    if (!payload) {
      payload = await probeServerIntel(entry._server);
      await saveSnapshot(entry, payload);
    }
    const hist = db.isConfigured() ? await db.query(
      `SELECT probed_at, (payload->>'up')::boolean AS up,
              (payload->'probes'->'ping'->'data'->>'ms')::int AS ping_ms
         FROM intel_snapshots
        WHERE server_key=$1 AND probed_at > now() - interval '7 days'
        ORDER BY probed_at ASC LIMIT 200`, [key]) : { rows: [] };
    const health = await getHealthRollup();
    const healthRow = health.servers.find(h => normUrl(h.url) === entry.url) || null;
    const probeSummary = summarizeProbes(payload.probes);
    return {
      entry: stripSecrets(entry),
      payload,
      history: hist.rows.map(r => ({ at: r.probed_at, up: r.up })),
      pingSeries: hist.rows.map(r => ({ at: r.probed_at, ms: r.ping_ms })).filter(r => r.ms != null),
      health: healthRow,
      probeSummary,
    };
  }

  async function refreshServer(key) {
    const inventory = await buildInventory();
    const entry = inventory.find(e => e.key === key);
    if (!entry) return null;
    const payload = await probeServerIntel(entry._server);
    await saveSnapshot(entry, payload);
    _lastSnapshotAt = new Date().toISOString();
    _cache.invalidate();
    return {
      entry: stripSecrets(entry),
      payload,
      probeSummary: summarizeProbes(payload.probes),
      at: _lastSnapshotAt,
    };
  }

  let _liveCache = null;
  let _liveCacheAt = 0;
  const LIVE_CACHE_MS = 15000;

  async function getLiveIntel() {
    if (_liveCache && Date.now() - _liveCacheAt < LIVE_CACHE_MS) return _liveCache;
    if (!liveSessions || !db.isConfigured()) {
      const empty = { live: [], count: 0, at: new Date().toISOString() };
      _liveCache = empty;
      _liveCacheAt = Date.now();
      return empty;
    }
    const q = await db.query(
      `SELECT u.id, u.username FROM users u
        JOIN user_config uc ON uc.user_id = u.id
       WHERE jsonb_array_length(uc.config_json->'servers') > 0`);
    const sessions = [];
    await Promise.all(q.rows.map(async (row) => {
      try {
        const cfg = await userConfig.getForServe(row.id);
        const live = await liveSessions.forUser(cfg?.servers || []);
        for (const s of live) sessions.push({ ...s, bridgeUserId: row.id, bridgeUsername: row.username });
      } catch { /* */ }
    }));
    const out = { live: sessions, count: sessions.length, at: new Date().toISOString() };
    _liveCache = out;
    _liveCacheAt = Date.now();
    return out;
  }

  async function getTokensIntel() {
    const cache = tokenCacheEntries();
    const events = await recentEvents(80);
    const agg = await aggregateEvents();
    const series = await reauthTimeSeries(7);
    const inventory = await buildInventory();
    const missingCreds = inventory.filter(e => !e.hasReauthCredentials).map(stripSecrets);
    const failureLeaderboard = Object.entries(agg.byServer || {})
      .filter(([, v]) => v.fail > 0)
      .map(([url, v]) => ({ url, label: v.label, ok: v.ok, fail: v.fail }))
      .sort((a, b) => b.fail - a.fail)
      .slice(0, 12);
    return { cache, events, aggregate: agg, series, missingCreds, failureLeaderboard };
  }

  async function getActivityIntel(range = '7d') {
    const days = range === '24h' ? 1 : 7;
    let logRows = [];
    if (db.isConfigured() && requestLog) {
      try {
        const q = await db.query(
          `SELECT ts, content_name, type, season, episode, best_server, found, response_ms, user_id
             FROM request_log WHERE ts > now() - ($1::int || ' days')::interval ORDER BY ts DESC LIMIT 8000`,
          [days]);
        logRows = q.rows.map(r => ({
          ts: r.ts, contentName: r.content_name, type: r.type, season: r.season, episode: r.episode,
          bestServer: r.best_server, found: r.found, ms: r.response_ms, userId: r.user_id,
        }));
      } catch { /* */ }
    }
    if (!logRows.length) {
      logRows = getRequestLog().map(e => ({
        ts: e.ts, contentName: e.contentName, type: e.type, bestServer: e.bestServer,
        found: e.found, ms: e.ms, userId: e.userId,
      }));
    }
    const windowMs = days === 1 ? 86400000 : 7 * 86400000;
    const latencyBuckets = new Map();
    for (const e of logRows) {
      if (e.ms == null) continue;
      const key = new Date(e.ts).toISOString().slice(0, days === 1 ? 13 : 10);
      if (!latencyBuckets.has(key)) latencyBuckets.set(key, { label: key, sum: 0, n: 0 });
      const b = latencyBuckets.get(key);
      b.sum += e.ms;
      b.n++;
    }
    const latencySeries = [...latencyBuckets.values()]
      .map(b => ({ label: b.label, avgMs: Math.round(b.sum / b.n) }))
      .sort((a, b) => a.label.localeCompare(b.label));
    const requests = timeSeries(logRows, { days, hourly: days === 1 });
    const errorRateSeries = requests.map(b => ({
      label: b.label,
      errorRate: b.total ? Math.round((b.failed / b.total) * 100) : 0,
      failed: b.failed,
      total: b.total,
    }));
    return {
      range: days === 1 ? '24h' : '7d',
      requests,
      errorRateSeries,
      servers: serverBreakdown(logRows, { windowMs }),
      topContent: topContent(logRows, { windowMs, limit: 12 }),
      latencySeries,
      total: logRows.length,
    };
  }

  async function getBridgeIntel(range = '24h') {
    const hours = range === '7d' ? 168 : 24;
    let series = [];
    if (db.isConfigured()) {
      try {
        const q = await db.query(`
          SELECT sampled_at, cpu_percent, sys_mem_pct, rss_bytes, heap_used_bytes, load_avg1, uptime_sec
            FROM bridge_metrics
           WHERE sampled_at > now() - ($1::int || ' hours')::interval
           ORDER BY sampled_at ASC`, [hours]);
        series = q.rows.map(r => ({
          at: r.sampled_at,
          cpu: r.cpu_percent,
          ram: r.sys_mem_pct,
          rss: Number(r.rss_bytes),
          heap: Number(r.heap_used_bytes),
          load: Number(r.load_avg1),
          uptime: r.uptime_sec,
        }));
      } catch { /* */ }
    }
    return {
      current: systemMetrics(),
      series,
      deployment: {
        node: process.version,
        platform: process.platform,
        arch: process.arch,
        region: process.env.RAILWAY_REGION || process.env.FLY_REGION || null,
        service: process.env.RAILWAY_SERVICE_NAME || null,
        env: process.env.NODE_ENV || 'development',
        hostname: os.hostname(),
      },
      services: {
        database: db.isConfigured(),
      },
    };
  }

  function triggerRefresh() {
    if (_refreshInFlight) return _refreshInFlight;
    _refreshInFlight = runSnapshotCycle().finally(() => { _refreshInFlight = null; });
    return _refreshInFlight;
  }

  return {
    buildInventory,
    probeServerIntel,
    runSnapshotCycle,
    saveBridgeMetric,
    pruneSnapshots,
    hydrateLastSnapshotAt,
    getIntelMeta,
    getStatus,
    getDashboard,
    getOverview,
    getServers,
    getServerDetail,
    refreshServer,
    getTokensIntel,
    getActivityIntel,
    getHealthRollup,
    getBridgeIntel,
    getLiveIntel,
    getRecentActivity,
    getFleetHeatmap,
    getDataDictionary,
    triggerRefresh,
    purgeDemoServersFromAllUsers,
    ensureDemoServersPurged,
    getLastSnapshotAt: () => _lastSnapshotAt,
    isRefreshInFlight: () => !!_refreshInFlight,
  };
}

module.exports = {
  makeAdminIntel,
  serverIntelKey,
  normUrl,
  probeServerIntel,
  summarizeProbes,
  makeTTLCache,
  getDataDictionary,
  tokenCacheEntries,
  TOKEN_TTL,
  SNAPSHOT_INTERVAL_MS,
  BRIDGE_METRIC_INTERVAL_MS,
  CACHE_TTL_MS,
};