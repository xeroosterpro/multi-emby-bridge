...
// Watched history logs the bridge's ranked #1 stream (pickedServer), not which
// server Stremio actually played. Live Sessions are the source of truth for
// "playing now" and we surface both labels honestly in the dashboard.
...
function enrichRecentEntries(recent, liveSessions) {
  return (recent || []).map((entry) => {
    ...
    const pickedServer = entry.server || null;
    const availableOn = availableServersFromStatus(entry.serverStatus);
    const playingServer = confirmedPlayingServer(playing);
    const liveUncertain = !!playing && !playingServer;
    const displayServer = playingServer
      || (availableOn.length === 1 ? (availableOn[0] || pickedServer) : null);
    return {
      ...entry,
      pickedServer,
      playingServer,
