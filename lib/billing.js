...
function makeBilling(db, events = null) {
  ...
  return {
    ...
    // Idempotency claim for inbound webhooks. Returns true if this event id is
    // newly recorded (process it), false if already seen (skip — a retry/dup).
    // Events with no id can't be deduped, so they're allowed through.
    async claimWebhook(eventId, eventType) {
      if (!eventId) return true;
      const r = await db.query(
        `INSERT INTO processed_webhooks(event_id, event_type) VALUES($1,$2)
           ON CONFLICT (event_id) DO NOTHING RETURNING event_id`,
        [String(eventId), eventType || null]
      );
      return r.rowCount > 0;
    },
