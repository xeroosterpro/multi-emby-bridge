...
function makeUserConfig(db) {
  ...
  return {
    async save(userId, fullConfig) {
      ...
      let merged = { ...existingCfg, ...incoming };

      const existingServers = Array.isArray(existingCfg.servers) ? existingCfg.servers : [];
      const incomingServers = incoming.servers;
      if (incomingServers === undefined) {
        merged.servers = existingServers;
      } else if (Array.isArray(incomingServers) && incomingServers.length === 0 && existingServers.length > 0) {
        merged.servers = existingServers;
      } else if (Array.isArray(incomingServers)) {
        merged.servers = incomingServers;
      }

      if (existing && Array.isArray(merged.servers) && merged.servers.length) {
        const existingCreds = decryptServerCreds(existing);
        merged.servers = mergeServerCreds(merged.servers, credsByUrl(existingCreds, merged.servers));
      }

      const serverCreds = extractServerCreds(merged.servers);
      let serversEnc = serverCreds.some(c => c.apiKey || c.username || c.password)
        ? crypto.encrypt(JSON.stringify(serverCreds))
        : null;
      if (!serversEnc && existing?.servers_enc) serversEnc = existing.servers_enc;
