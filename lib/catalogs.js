...
async function fetchTmdbImdbIds(tmdbIds, mediaType, apiKey) {
  ...
  for (var i = 0; i < tmdbIds.length; i += 20) {
    ...
    var resolved = await Promise.all(batch.map(async function(id) {
      try {
        var r = await timedFetch('https://api.themoviedb.org/3/'+ep+'/'+id+'/external_ids', { headers: { 'Authorization': 'Bearer '+apiKey } }, 8000);
        if (!r.ok) return null;
        var d = await r.json();
...
async function fetchTmdbCharts(tmdbChart, mediaType, apiKey) {
  ...
  for (var page = 1; page <= 5; page++) {
    ...
    var r = await timedFetch(url, { headers: { 'Authorization': 'Bearer '+apiKey } }, 8000);
    if (!r.ok) {
      if (page === 1) {
