...
const { assertSafeFetchUrl, safeAgent, SAFE_REDIRECT_LIMIT } = require("./urlSafety");

const CACHE_TTL = 10 * 60 * 1000;
const { makeLruCache } = require('./lruCache');
const _cache = makeLruCache({ max: 200, ttlMs: CACHE_TTL });
function cacheGet(key) { const v = _cache.get(key); return v === undefined ? null : v; }
function cacheSet(key, data) { _cache.set(key, data); }
async function timedFetch(url, opts, timeoutMs) { opts=opts||{}; timeoutMs=timeoutMs||10000; const c=new AbortController(); const t=setTimeout(()=>c.abort(),timeoutMs); try{return await fetch(url,Object.assign({agent:safeAgent,follow:SAFE_REDIRECT_LIMIT},opts,{signal:c.signal}));}finally{clearTimeout(t);} }

// TMDB supports two key types:
//   v4 Read Access Token: long JWT starting with "eyJ" — use Authorization: Bearer header
//   v3 API Key: 32-char hex string — must use ?api_key= query parameter
function tmdbFetchOpts(apiKey) {
  if (apiKey && apiKey.trim().startsWith('eyJ')) {
    return { headers: { 'Authorization': 'Bearer ' + apiKey.trim() } };
  }
  return {};
}
function appendTmdbKey(url, apiKey) {
  var key = apiKey ? apiKey.trim() : '';
...
async function fetchLetterboxdRssList(listUrl) {
  ...
  await assertSafeFetchUrl(rssUrl, 'letterboxd list url');
...
async function fetchTmdbImdbIds(tmdbIds, mediaType, apiKey) {
  ...
  for (var i = 0; i < tmdbIds.length; i += 20) {
    ...
    var resolved = await Promise.all(batch.map(async function(id) {
      try {
        var r = await timedFetch(appendTmdbKey('https://api.themoviedb.org/3/'+ep+'/'+id+'/external_ids', apiKey), tmdbFetchOpts(apiKey), 8000);
        if (!r.ok) return null;
        var d = await r.json();
...
async function fetchTmdbCharts(tmdbChart, mediaType, apiKey) {
  ...
  for (var page = 1; page <= 5; page++) {
    ...
    var r = await timedFetch(appendTmdbKey(url, apiKey), tmdbFetchOpts(apiKey), 8000);
    if (!r.ok) {
      if (page === 1) {
        if (r.status === 401) throw new Error('Invalid TMDB API key — use your API Read Access Token (starts with eyJ) or API Key from themoviedb.org Settings → API');
...
async function fetchTmdbDiscover(entry, apiKey) {
  ...
  var params = '?sort_by='+(entry.tmdbSortBy||'popularity.desc');
  ...
  if (entry.tmdbWatchProvider) {
    ...
    params += '&watch_region='+encodeURIComponent(entry.tmdbWatchRegion || 'US');
    // When filtering by streaming service but no explicit year range is set,
    // automatically limit to content from the last 3 years so old classics
    // don't crowd out currently popular titles (e.g. Breaking Bad on Netflix)
    if (!entry.tmdbYearFrom && !entry.tmdbYearTo) {
      var autoYear = new Date().getFullYear() - 3;
      var autoDateGte = autoYear + '-01-01';
      if (mediaType === 'series') params += '&first_air_date.gte=' + encodeURIComponent(autoDateGte);
      else params += '&primary_release_date.gte=' + encodeURIComponent(autoDateGte);
    }
  }
...
// Fetch TMDB weekly trending, then post-filter to titles available on a specific
// streaming service. This gives a much more "trending on Netflix/Prime/etc."
// result than Discover+popularity which surfaces old classics.
async function fetchTmdbTrendingFiltered(mediaType, watchProvider, watchRegion, apiKey) {
  if (!apiKey) throw new Error('TMDB API key is required.');
  var ep = mediaType === 'series' ? 'tv' : 'movie';
  var region = watchRegion || 'US';
  var provId = String(watchProvider);

  var cacheKey = 'tmdb:trendingprov:'+ep+':'+provId+':'+region;
  var cached = cacheGet(cacheKey); if (cached) return cached;

  // Step 1: fetch trending/week (up to 3 pages = 60 titles)
  var allTrending = [];
  for (var page = 1; page <= 3; page++) {
    var url = 'https://api.themoviedb.org/3/trending/'+ep+'/week?page='+page+'&language=en-US';
    var r = await timedFetch(appendTmdbKey(url, apiKey), tmdbFetchOpts(apiKey), 8000);
    if (!r.ok) {
      if (page === 1) {
        if (r.status === 401) throw new Error('Invalid TMDB API key — use API Key or API Read Access Token from themoviedb.org Settings → API');
        throw new Error('TMDB trending returned '+r.status);
      }
      break;
    }
    var data = await r.json();
    ...
    allTrending = allTrending.concat(results);
  }

  // Step 2: batch-check watch providers for each trending title
  var filtered = [];
  for (var i = 0; i < allTrending.length; i += 20) {
    var batch = allTrending.slice(i, i + 20);
    var checks = await Promise.all(batch.map(async function(item) {
      try {
        var provUrl = 'https://api.themoviedb.org/3/'+ep+'/'+item.id+'/watch/providers';
        var pr = await timedFetch(appendTmdbKey(provUrl, apiKey), tmdbFetchOpts(apiKey), 5000);
        if (!pr.ok) return null;
        var provData = await pr.json();
        var regionData = (provData.results || {})[region];
        if (!regionData) return null;
        // Check flatrate (subscription) and free tiers
        var providers = (regionData.flatrate || []).concat(regionData.free || []);
        var hasIt = providers.some(function(p) { return String(p.provider_id) === provId; });
        return hasIt ? item : null;
      } catch(e) { return null; }
    }));
    checks.forEach(function(item) { if (item) filtered.push(item); });
  }

  // Step 3: resolve IMDb IDs (trending order is preserved)
  var tmdbIds = filtered.map(function(r){ return r.id; });
  var idMap = await fetchTmdbImdbIds(tmdbIds, mediaType, apiKey);
...
async function fetchExternalCatalog(entry, rpdbKey, traktClientId, catalogLang, tmdbApiKey) {
  ...
  if (entry.provider==='trakt') {
    ...
    if (entry.tmdbMode==='trending-provider') {
      items=await fetchTmdbTrendingFiltered(entry.mediaType==='series'?'series':'movie', entry.tmdbWatchProvider, entry.tmdbWatchRegion||'US', tmdbApiKey);
    } else if (entry.tmdbMode==='discover') {
