...
async function fetchTraktList(listType, mediaType, clientId) {
  ...
  const resp = await timedFetch(url, { headers: { 'Content-Type':'application/json','trakt-api-version':'2','trakt-api-key':clientId } });
...
async function fetchMdblistItems(listUrl, apiKey) {
  ...
  const rr = await timedFetch('https://mdblist.com/api/?apikey='+encodeURIComponent(apiKey)+'&url='+encodeURIComponent(listUrl));
  if (!rr.ok) throw new Error('MDbList resolve returned '+rr.status);
  const rd = await rr.json(); const listId = rd.id;
  if (!listId) throw new Error('Could not resolve MDbList list ID from URL.');
  const ir = await timedFetch('https://mdblist.com/api/lists/'+listId+'/items/?apikey='+encodeURIComponent(apiKey)+'&limit=20');
  if (!ir.ok) throw new Error('MDbList items returned '+ir.status);
  const id2 = await ir.json();
  const rows = id2.movies||id2.shows||id2.items||[];
  const items = rows.map(function(item){
    return { imdbId:item.imdb_id||null, title:item.title||null, year:item.year||null, type:item.mediatype==='show'?'series':'movie' };
  }).filter(function(i){return i.imdbId;});
  cacheSet(cacheKey, items); return items;
}
